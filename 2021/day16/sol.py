...
def hex_to_bin_encoding(hex_rep: str) -> str:
    def _encoding(ch: str) -> str:
        hex_digit = int(ch, 16) # hex to base 10 digit
        bin_digit = bin(hex_digit)[2:]
        return bin_digit.rjust(4, '0') # right justify with 0

    return ''.join([_encoding(ch) for ch in hex_rep])
...
def extract_from_hex(hex_seq):
    bit_seq = hex_to_bin_encoding(hex_seq)
    return extract_from_bin(bit_seq)


def extract_from_bin(bit_seq: int):
    pointer = 0
    # Find the header
    while pointer < len(bit_seq):
        pv = bit_seq[pointer:pointer+3]
        pt = bit_seq[pointer+3:pointer+6]
        ...
        if pt == '100':  # id: 4
            group_val = ''
            while True:
                group = bit_seq[pointer:pointer + 5]
                pointer += 5
                group_val += group[1:]
                if group[0] == '0':
                    packet.bits = bit_seq[packet.start_bit:pointer]
                    packet.literal_value = int(group_val, 2)
                    return packet

        # Otherwise, create sub-packets (recursion FTW!)
        else:
            length_type_id = int(bit_seq[pointer], 2)
            pointer += 1
            if length_type_id == 0:  # length reps number of bits
                sub_pack_length = int(bit_seq[pointer:pointer+15], 2)
                pointer += 15
                sub_packets = list()
                pointer_2 = 0
                while pointer_2 < sub_pack_length:
                    sub_bin = bit_seq[pointer+pointer_2:]
            ...
            if length_type_id == 1:  # length reps number of packets
                sub_pack_count = int(bit_seq[pointer:pointer+11], 2)
                pointer += 11
                sub_packets = list()

                while len(sub_packets) < sub_pack_count:
                    sub_bin = bit_seq[pointer:]
                    sub_pack = extract_from_bin(sub_bin)
                    sub_packets.append(sub_pack)
...
def tests():
    ...
    for h, b in conversion_tests:
        test_result = hex_to_bin_encoding(h)
